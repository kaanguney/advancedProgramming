...
int main() {
        ...
        for(int j=0; j<inputfiles.size(); j++) { // search in each file
            ...
            while(getline(input, line)) {
                ...
                while(iss >> word) {
                    ...
                    query[i] = StripPunc(query[i]);
                    query[i] = make_upper(query[i]); // make all chars uppercase
    ...
    for(int i=0; i<query.size(); i++) {
        ...
        for(int j=0; j<inputfiles.size(); j++) { // search in each file
            ...
            if(table.find(query[i])) {
                
                WordItem *ptr = table.elts(query[i]);
                
                ptr ->info[j].count = wordcount;
                ptr->word = query[i];
            }
